...
*Libraries are added for future computations, dates are set as Date Format*
...
*As you may notice, the shape of the histogram is very similar to the one of the missing values, this is due
NA's values represent about 13% of  the total values, so the effect of them is low in the general tendence 
of data. Nevertheless the next values show that replacing the missing values with the respective 5 min interval
mean, improve the simetry of the curve (less bias) so now the median value correspond to the mean*

```{r}
print(c("mean"=mean.ds1,"median"=median.ds1))
```

*In order to have a better visualization , I included both histograms in the plot below.*
